...
![Version](https://img.shields.io/badge/version-1.1.87-blue)
...
- The support modal keeps both PayPal and Ko-fi available while using official external pages for checkout/support

### Highlights in `v1.1.86`

- Tightened the footer support row again so Donate and Support me on Ko-fi fit side by side more reliably in narrow layouts
- Reduced both support buttons evenly so they stay identical while taking less space
...
- Donate and Support me on Ko-fi now use identical compact dimensions and remain side by side in the footer instead of wrapping unevenly
- Tightened the support row so the footer feels more balanced on mobile and desktop
...
- Mobile active calls now auto-restore cleanly after background/minimized states, while explicit close attempts still preserve the recovery decision flow on next launch
- Removed shell overflow rules that were making vertical dashboard scrolling feel sticky or dependent on sideways gestures first

### Highlights in `v1.1.81`

- Replaced the embedded Ko-fi widget with a fixed button so the footer support actions stay visually consistent and no external widget stretches the layout
- Donate and Support me on Ko-fi now sit side by side with matching rounded button dimensions
